...
export const excludeType = [ 'jsonb' ];
export const excludeName = [ 'Geometry' ];
export const excludeOptionName = 'excludeZombodb';
...
type Fields = Array<{type:string, key: string}>;

export function indexing(
    schemaName: string, 
    tableName: string, 
    useFieldsOrFunc: Fields|string
){
    schemaName = schemaName || 'public';

    if(typeof (<string>useFieldsOrFunc) === 'string'){
        return `
        ...
        return `
            CREATE TYPE "${schemaName}"."${tableName}_row_type" AS (${
                (<Fields>useFieldsOrFunc).map((v) => { 
                    return `"${v.key}" ${v.type}`; 
                }).join(', ')
            });
